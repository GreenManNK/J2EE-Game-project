...
$fullstackRoot = Split-Path -Parent $repoRoot
$workspaceRoot = Split-Path -Parent $fullstackRoot
...
Invoke-Check "IntelliJ Start (Default Public)" {
    Assert-IntelliJRunConfig "Start (Default Public).run.xml" '$PROJECT_DIR$/FULLSTACK/BACKEND/scripts/manual-start.cmd' '--no-pause'
} | Out-Null
Invoke-Check "IntelliJ Stop (All)" {
    Assert-IntelliJRunConfig "Stop (All).run.xml" '$PROJECT_DIR$/FULLSTACK/BACKEND/scripts/manual-start.cmd' 'stop --no-pause'
} | Out-Null

Write-Step "Kiem tra nut bam VS Code tasks"
Invoke-Check "VS Code tasks.json public task mappings" {
    $tasksPath = Join-Path $workspaceRoot ".vscode/tasks.json"
