(function () {
  ...
  function setCurrentUser(user) {
    ...
    const previousStored = readStoredCurrentUser();
    const previous = previousStored && previousStored.userId ? normalizeCurrentUser(previousStored) : null;
    ...
    if (previous && previous.userId === normalized.userId && normalized.score > previous.score) {
      announceRewardScoreGain(normalized.score - previous.score);
    }
  }
  ...
  function pulseRewardUi() {
    const rewardBadges = document.querySelectorAll('[data-current-user-reward]');
    if (!rewardBadges.length) {
      return;
    }
    rewardBadges.forEach((el) => el.classList.add('is-earned'));
    if (rewardPulseTimer) {
      window.clearTimeout(rewardPulseTimer);
    }
    rewardPulseTimer = window.setTimeout(() => {
      rewardBadges.forEach((el) => el.classList.remove('is-earned'));
    }, 1400);
  }

  function announceRewardScoreGain(delta) {
    const gain = normalizeNonNegativeInt(delta, 0);
    if (gain <= 0) {
      return;
    }
    pulseRewardUi();
    if (window.CaroUi && typeof window.CaroUi.toast === 'function') {
      window.CaroUi.toast('Diem thuong +' + String(gain), {
        type: 'success',
        title: 'Thuong PvP'
      });
    }
  ...
  document.addEventListener('DOMContentLoaded', () => {
    ...
    const userScores = document.querySelectorAll('[data-current-user-score]');
    ...
    const applyAuthState = (user) => {
      ...
      userScores.forEach((el) => {
        el.textContent = String(safeUser ? normalizeNonNegativeInt(safeUser.score, 0) : 0);
      });
