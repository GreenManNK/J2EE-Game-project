(function () {
  ...
  function initRealtimeRoomUpdates() {
    ...
    roomRealtimeClient = new window.StompJs.Client({
      ...
      onStompError: () => {
        setRoomListNote('Nguon du lieu: realtime loi, dang dung API');
        void loadRooms(true);
      },
      onWebSocketError: () => {
        setRoomListNote('Nguon du lieu: mat realtime, dang dung API');
        void loadRooms(true);
      },
      onWebSocketClose: () => {
        setRoomListNote('Nguon du lieu: dang thu ket noi lai');
      }
    });
  ...
  function bindRealtimeLifecycle() {
    const resume = () => {
      if (navigator.onLine === false) {
        setRoomListNote('Nguon du lieu: dang offline');
        return;
      }
      if (roomRealtimeClient && !roomRealtimeClient.active) {
        try {
          roomRealtimeClient.activate();
        } catch (_) {
        }
