package com.game.hub.chess.websocket;
...
public class ChessWebSocketController {
    ...
    public void join(ChessJoinMessage message, SimpMessageHeaderAccessor headers) {
        if (message == null) {
            return;
        }
    ...
    public void move(ChessMoveMessage message, SimpMessageHeaderAccessor headers) {
        if (message == null) {
            return;
        }
        String roomId = safeTrim(message.getRoomId());
        ...
        ChessOnlineRoomService.ActionResult result = roomService.move(
            roomId,
            userId,
            message.getFromRow(),
            message.getFromCol(),
    ...
    public void reset(ChessRoomActionMessage message, SimpMessageHeaderAccessor headers) {
        if (message == null) {
            return;
        }
        String roomId = safeTrim(message.getRoomId());
        ...
        ChessOnlineRoomService.ActionResult result = roomService.resetGame(roomId, userId);
        if (!result.ok()) {
            sendUserError(roomId, userId, result.error());
            ChessOnlineRoomService.RoomSnapshot room = roomService.roomSnapshot(roomId);
            if (room != null) {
    ...
    public void leave(ChessRoomActionMessage message, SimpMessageHeaderAccessor headers) {
        ...
        ChessOnlineRoomService.RoomSnapshot room = roomService.roomSnapshot(roomId);
        if (room == null) {
            messagingTemplate.convertAndSend("/topic/chess.room." + roomId, Map.of(
                "type", "ROOM_CLOSED",
                "roomId", roomId,
    ...
    public void onSessionDisconnect(SessionDisconnectEvent event) {
        ...
        ChessOnlineRoomService.RoomSnapshot room = roomService.roomSnapshot(presence.roomId());
        if (room == null) {
            messagingTemplate.convertAndSend("/topic/chess.room." + presence.roomId(), Map.of(
                "type", "ROOM_CLOSED",
    ...
    private void broadcastRoomList() {
        messagingTemplate.convertAndSend("/topic/chess.rooms", Map.of(
            "type", "ROOM_LIST",
            "rooms", roomService.availableRooms()
        ));
    }

    private void broadcastRoomState(String roomId, String type, ChessOnlineRoomService.RoomSnapshot room, String message) {
    ...
    private void rememberRoomPresence(SimpMessageHeaderAccessor headers, String roomId, String userId) {
        ...
        ChessOnlineRoomService.RoomSnapshot room = roomService.roomSnapshot(previous.roomId());
        if (room != null) {
