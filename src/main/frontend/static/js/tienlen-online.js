(function () {
  ...
  function initClient() {
    ...
    const client = new window.StompJs.Client({
      ...
      onConnect: () => {
        ...
        setStatus(state.roomId ? ('Da ket noi lai. Dang dong bo phong ' + state.roomId + ' ...') : 'Da ket noi. Co the vao phong.');
        subscribeSharedChannels(client);
        requestRoomList();
        const roomToSync = String(state.roomId || els.roomInput?.value || '').trim();
        if (roomToSync) {
          syncCurrentRoomAfterReconnect(roomToSync);
        } else {
          updateActionButtons();
        }
      },
  ...
  function bindRealtimeLifecycle() {
    const resume = () => {
      if (!state.client || navigator.onLine === false) {
        return;
      }
      if (!state.connected) {
        setStatus(state.roomId
          ? ('Internet da quay lai. Dang dong bo phong ' + state.roomId + ' ...')
          : 'Internet da quay lai. Dang khoi phuc ket noi...');
      }
  ...
  function publishJoinForRoom(roomId) {
    if (!roomId) {
      return;
    }
  ...
  function syncCurrentRoomAfterReconnect(roomId) {
    const normalizedRoomId = String(roomId || '').trim();
    if (!normalizedRoomId) {
      return;
    }
    state.roomId = normalizedRoomId;
    if (els.currentRoomLabel) {
      els.currentRoomLabel.textContent = normalizedRoomId;
    }
    subscribeRoomTopic(normalizedRoomId);
    publishJoinForRoom(normalizedRoomId);
    renderAll();
  }
  ...
  function renderCardElement(card, selected, readonly) {
    ...
    const spriteStyle = cardSpriteStyleForCode(code);
    if (spriteStyle) {
